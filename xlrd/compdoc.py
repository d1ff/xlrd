...
# 2007-04-22 SJM Missing "<" in a struct.unpack call => can't open files
...
class CompDoc(object):
    ...
    def __init__(self, mem, logfile=sys.stdout, DEBUG=0, suppress_locate_stream_error=False):
        self.logfile = logfile
        self.DEBUG = DEBUG
        ...
        if MSATX_tot_secs == 0 and MSATX_first_sec_sid in (EOCSID, FREESID, 0):
            ...
            # should be set to EOCSID ... FREESID and 0 have been met in the
            # wild.
        ...
        for msidx in xrange(len(MSAT)):
            ...
            if msid in (FREESID, EOCSID):
                ...
                # Toleration: a FREESID or EOCSID entry anywhere in the MSAT
    ...
    def locate_named_stream(self, qname):
        ...
        if d is None:
            return (None, 0, 0)
        if d.tot_size > self.mem_data_len and not self.suppress_locate_stream_error:
    ...
    def _locate_stream(self, mem, base, sat, sec_size, start_sid, expected_stream_size, qname, seen_id):
        ...
        while s >= 0:
            if self.seen[s] and not self.suppress_locate_stream_error:
